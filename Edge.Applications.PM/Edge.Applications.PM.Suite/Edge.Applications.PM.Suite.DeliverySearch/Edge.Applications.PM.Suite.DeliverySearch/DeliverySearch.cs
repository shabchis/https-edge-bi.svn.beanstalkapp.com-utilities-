...
{
	public partial class DeliverySearch : Edge.Applications.PM.Common.ProductionManagmentBaseForm
	{
		public DeliverySearch()
		{
			InitializeComponent();
			this.fromDate.Value = DateTime.Today.AddDays(-1);
			this.toDate.Value = DateTime.Today.AddDays(-1);
			searchParamsPanel.Enabled = false;
		}
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			using (SqlConnection sqlCon = new SqlConnection(AppSettings.GetConnectionString(this, "DeliveryDB")))
			{
				...
				string query1 = "SELECT DeliveryID,AccountID,OriginalID,ChannelID,DateCreated,TargetPeriodStart,TargetPeriodEnd,[Committed] from dbo.Delivery " +
				   " where TargetPeriodStart between @TargetPeriodStart and @TargetPeriodEnd ";
				queryBuilder.Append(query1);

				if (commitedOption.Checked)
				{
					queryBuilder.Append(" and Committed = 1 ");
				}
		...
		private void application_cb_SelectedValueChanged(object sender, EventArgs e)
		{
			...
			{
				key = Const.AdMetricsConst.EdgeProductionPathKey;
			}
			else
				key = Const.AdMetricsConst.SeperiaProductionPathKey;
